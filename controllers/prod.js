...
productos.listarproductoslaboxfase = async (req, res) => {
  const fase = req.params.p1;

  try {
    const resultado = await (
      await pool.query(
        "select * from proyectoerp.erp_listarproductoslaboxfase($1)",
        [fase]
      )
    ).rows;
...
productos.avanzarfaseprod = async (req, res) => {
  const idprod = req.params.p1;

  try {
    await pool.query("select proyectoerp.cambiar_fase_producto($1)", [
      idprod
    ]);
